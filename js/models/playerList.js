...
define([
    ...
    var Player = Backbone.Model.extend({
        ...
        isPreviousOpponent: function (player, roundLookBack) {
            ...
            return _.any(this.getPreviousOpponents(roundLookBack), function (oppid) {
                return oppid === pid;
            });
        },
        ...
        getPossibleFirstRoundOpponents: function (players) {
            ...
            var possibleOpps = _.filter(players, function (player) {
                if (that.id === player.id) {
                    return false;
                }
                if (that.getCity() !== '' && that.getCity() === player.getCity()) {
                    return false;
                }
                if (that.getFaction() !== '' && that.getFaction() === player.getFaction()) {
                    return false;
                }
                return true;
            });
        ...
        getValueForRound: function (valueType, round) {
            ...
            var rounds = round;
            while (this.get(valueType + (rounds + 1))) {
                rounds++;
            }
            if (value !== BYE_SCORE) {
        ...
        getOpponentIdForRound: function (round) {
            return this.get('opponent' + round);
        },
        getOpponentForRound: function (round) {
            return this.collection.get(this.getOpponentIdForRound(round));
        },
        setOpponentIdForRound: function (round, opponent) {
